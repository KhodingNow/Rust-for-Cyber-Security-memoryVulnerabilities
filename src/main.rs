...
THEN... we escalate priviledges with a Closure + Rust specialist iterative methods

- Higher order functions while still respecting ownership, lifetimes, borrowing
- Show how data flows through a Closure safely - no memory leaks or triggering of race conditions
...
//1. A simple vector
      let mut data = [200, 400, 600];
     println!("Original Vec: {data:?}");

//2. Closure with .iter() for read only, this method does not mutate, move, nor owns heap data.
...
// 3. Closure with .into_iter() - this method can move, mutate and OWNS the data
     ...
     println!("{owned_vec:?}"); // STILL safe not MOVED  - as long as .into_iter() does not consume the vector first...TRY comment .iter() to demo error
...
// 4. map() + collect() avoids intermediate allocations
    let tripled = [5, 6, 7, 8]
     ...
     println!("\n .iter() + .map() + .collect(): tripled = {tripled:?}");

// 5. Show safety in borrowed context

    let secured = ["admin", "user", "root"];
    println!("\n Checking the length safely with a closure:");

    #[allow(clippy::for_each)] // SILENCING Clippy :)
...
// 6. Induced error - invalid memory location access
    ...
    let vet = [2, 4, 6, 8];
    //println!("{}", vet[1001]); // Panic: prevents overflowing the buffer 

// 7. Rust prevents race conditions with closure scope
    
    let sensitive = [40, 1332, 800];
...
// fn main() {
        let new_vec = [8, 9, 0, 3, 4, 6, 7, 11, 3, 5, 2, 20, 13, 17]; // just a vec with numbers
        ...
        for index in 0..20 {
            empty_vec.push(
                new_vec
                    .get(index)
                    .map(|number| number + 1)
                    .map(|number| number + number_to_add)
            );
        }
        println!("{empty_vec:?}");        
   // }    
   ...
   // SIMULATED Attacker Injects PACKETS between two communicating devices
   // fn main() {
...
//    // Sender Thread 
    ...
    let sender_handle = thread::spawn(move || {
        ...
        for i in 1..=5 {
            ...
            println!("[Sender] Sending original data: {original_data}, MAC: {mac}");
...
//     // POISONER thread (Malicious actor)
    ...
    let poisoner_handle = thread::spawn(move || {
        ...
        while let Ok((data_from_sender, mac_from_sender)) = rx_sender_poisoner.recv() {
            println!("[Poisoner] Intercepting Data: {data_from_sender}, MAC: {mac_from_sender}");
...
// //     // RECEIVER thread
    ...
    let receiver_handle = thread::spawn(move || {
        ...
        while let Ok((received_data, received_mac)) = rx_poisoner_receiver.recv() {
            ...
            println!("[Receiver] Received Data: {received_data}, Received MAC: {received_mac}");
